...
namespace ResourceServices.Service
{
    public class ConfigurationDAO
    {
        ...
        public static List<ConfigurationDTO> GetConfigurationForEmployee(int companyId)
        {
            using (DeverateContext db = new DeverateContext())
            {
                return db.Configuration.Where(x => x.EndDate.Value >= DateTime.UtcNow.AddHours(1)  && x.Account.CompanyId == companyId && x.IsActive == true && x.Type == true).Select(x => new ConfigurationDTO(x)).ToList();
            }
        }
